...
const Lobby = props => {
  ...
  props.Lobby.lobby.forEach(room => {
    if (room.players.includes(props.Player.name)) multiple = true;
  });
  props.Lobby.lobby.forEach(room => {
    if (room.players.includes(props.Player.name)) myRoom = room;
  });
  let inGame;
        ...
        <div className="rpgui-content">
          <form className="rpgui-container framed customForm">
            <button
              onClick={e => {
                e.preventDefault();
                props.makeRoom(props.Player.name);
              }}
              disabled={multiple}
            >
              Make a room{" "}
                  ...
                  (!room.start
                    ? <div key={room.id}>
                        ...
                        {room.players &&
                          ...
                          : room.players && room.players.length < 4
                              ? <button
                                  disabled={multiple || room.start}
                                  onClick={e => {
                                    e.preventDefault();
                                    props.joinRoom(room.id, props.Player.name);
                                  }}
                                >
                                  Join Room
                                </button>
                        ...
                        {room.players.includes(props.Player.name)
                          ? <button
                              onClick={() =>
                                props.leaveRoom(room.id, props.Player.name)}
                            >
                              Leave Room
                            </button>
                          : <div />}
