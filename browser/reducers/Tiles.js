...
const reducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case REMOVE_PAINT:
      for (let i = 0; i < newState.crates.length; i++) {
        for (let j = 0; j < newState.crates[i].length; j++) {
          if (
            newState.crates[i][j].paint &&
            newState.crates[i][j].paint.key === action.color
          ) {
    ...
    case ADD_PAINT:
      if (
        !(action.paint.key ===
          newState.crates[action.payload.x][action.payload.y].paint.key)
      ) {
        if (newState.crates[action.payload.x][action.payload.y].paint)
          newState.crates[action.payload.x][action.payload.y].paint.kill();
        newState.crates[action.payload.x][action.payload.y] = Object.assign(
          {},
          newState.crates[action.payload.x][action.payload.y],
          { paint: action.paint }
        );
      }
      break;
    case ADD_FLAMES:
      if (
        typeof newState.crates[action.payload.x][action.payload.y].flame ===
        "object"
      )
      ...
      setTimeout(function() {
        if (newState.crates[action.payload.x][action.payload.y].flame)
          newState.crates[action.payload.x][action.payload.y].flame.kill();
        newState.crates[action.payload.x][action.payload.y].flame = false;
      }, 500);
      break;
    case ADD_POWERUP:
      newState.crates[action.powerXY.x][action.powerXY.y].powerUp =
        action.powerUp;
    ...
    case ADD_BOMB:
      if (!newState.crates[action.payload.x][action.payload.y].bomb) {
        newState.crates[action.payload.x][action.payload.y].bomb = action.bomb;
        newState.bombs.push(action.bomb);
      }
      break;
    case REMOVE_BOMB:
      newState.bombs = newState.bombs.filter(
        bomb =>
          bomb !== newState.crates[action.payload.x][action.payload.y].bomb
      );
      if (newState.crates[action.payload.x][action.payload.y].bomb) {
