...
export default class Hero {
  constructor(game, name, color) {
    ...
    socket.on("server_delete_timer", data => {
      if (
        store.getState().Player.name !== data.name &&
        this.game.game.lobby.id === data.LobbyId &&
        socket.id === data.socket
      ) {
        this.game.time.events.remove(
          store.getState().Tiles.crates[data.x][data.y].bomb.timer
        );
        this.bombs.pop();
      }
    });
  ...
  reset() {
    ...
    totalDrops.forEach(powerUp => {
      ...
      socket.emit("client_make_power", {
        ...
        socket: this.name,
        mySocket: socket.id,
        LobbyId: this.game.game.lobby.id
      });
  ...
  addSprite() {
    switch (this.color) {
      case "blue":
        ...
        this.avatar =
          "http://vignette1.wikia.nocookie.net/ssb/images/0/03/LarryTrophy3DS.png/revision/latest?cb=20140929194701&format=webp";
      ...
      case "purple":
        ...
        this.avatar =
          "http://vignette3.wikia.nocookie.net/ssb/images/4/4a/LemmyTrophy3DS.png/revision/latest?cb=20140929194817&format=webp";
        store.dispatch(addAvatar(this.avatar));
        store.dispatch(addMultiplayerAvatar(this.color, this.avatar));
      ...
      case "green":
        ...
        this.avatar =
          "http://vignette4.wikia.nocookie.net/ssb/images/5/56/Yoshi_%2B_Egg_1.png/revision/latest?cb=20140929214641&format=webp";
        store.dispatch(addAvatar(this.avatar));
        store.dispatch(addMultiplayerAvatar(this.color, this.avatar));
      ...
      case "red":
        ...
        this.avatar =
          "http://vignette4.wikia.nocookie.net/ssb/images/e/e1/BowserJrEXTrophy3DS.png/revision/latest?cb=20140929203914&format=webp";
        store.dispatch(addAvatar(this.avatar));
        store.dispatch(addMultiplayerAvatar(this.color, this.avatar));
  ...
  update(game) {
    ...
    this.game.physics.arcade.collide(this.sprite, this.fire, () => {
      if (this.immuneTime < this.game.time.now) {
        ...
        socket.emit("client_remove_paint", {
          color: this.color,
          socket: this.name,
          mySocket: socket.id,
          LobbyId: this.game.game.lobby.id
    ...
    if (this.immuneTime > this.game.time.now) {
      if (this.animation != "dead") this.sprite.animations.play("dead");
      this.animation = "dead";
      this.sprite.body.velocity.setTo(0, 0);
    } else {
      ...
      if (this.space.isDown) {
        if (!this.onePress) {
          if (this.bombs.length < this.limit) {
            ...
            let gridCoords = Utils.mapCoordsToGrid(
              blockCoords.x,
              blockCoords.y
            );
            if (!(this.immuneTime > this.game.time.now)) {
              if (
                !store.getState().Tiles.crates[gridCoords.x][gridCoords.y].bomb
              ) {
                socket.emit("client_place_bomb", {
                  x: blockCoords.x,
                  y: blockCoords.y,
                  range: this.range,
                  socket: this.name,
  ...
  explosion(gridCoords, blockCoords, myBomb, time) {
    myBomb.timer = this.game.time.events.add(Phaser.Timer.SECOND * time, () => {
      socket.emit("client_bomb_explode", {
        x: gridCoords.x,
        y: gridCoords.y,
        socket: this.name,
        mySocket: socket.id,
      ...
      let cratesToKill = Utils.adjacentCrates(
        blockCoords.x,
        blockCoords.y,
        myBomb.range,
        allCrates
      ...
      cratesToKill.forEach(crate => {
        if (allCrates[crate.x] && allCrates[crate.x][crate.y].crate === false) {
          ...
          if (
            allCrates[crate.x][crate.y].bomb &&
            allCrates[crate.x][crate.y].bomb !== myBomb
          ) {
            this.game.time.events.remove(
              allCrates[crate.x][crate.y].bomb.timer
            );
            socket.emit("client_delete_timer", {
              x: crate.x,
              y: crate.y,
              socket: allCrates[crate.x][crate.y].bomb.socket,
              name: this.name,
              LobbyId: this.game.game.lobby.id
            });
            this.explosion(
              { x: crate.x, y: crate.y },
              { x: crate.x * 48 + 24, y: crate.y * 48 + 24 },
              allCrates[crate.x][crate.y].bomb,
              0
            );
          }
        ...
        if (
          allCrates[crate.x] &&
          allCrates[crate.x][crate.y] &&
          allCrates[crate.x][crate.y].crate !== false
        ) {
          socket.emit("client_remove_crate", {
            x: crate.x,
            y: crate.y,
            socket: this.name,
            mySocket: socket.id,
          ...
          allCrates[crate.x][crate.y].crate.animations.add(
            "crateExplosion_ANI"
          );
          allCrates[crate.x][crate.y].crate.animations.play(
            "crateExplosion_ANI",
            30,
            false
          );
          ...
          if (powerUpChance === 1) {
            ...
            let randomPowerUp =
              randomPowerUpArray[
                Math.floor(Math.random() * randomPowerUpArray.length)
              ];
            ...
            let power = this.powerGroup.create(
              powerXY.x,
              powerXY.y,
              randomPowerUp
            );
  ...
  updateSocket() {
    socket.emit("client_data_transfer", {
      position: this.sprite.position,
      color: this.color,
      score: this.score,
      name: this.name,
      animation: this.animation,
      id: this.game.game.lobby.id
    });
  }
}
